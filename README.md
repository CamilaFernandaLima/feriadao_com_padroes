...
3. Adapter: vou supor que a loja ainda tem contrato com um sistema de cartão antigo, que usa um método chamado Transacao(). Assim, foi criado um AdaptadorPagamento, para uqe ele se encaixe na interface que simplesmente pede um Pagar().
4. Proxy: para os pedidos que já estiverem na cozinha, não é qualquer um que pode realizar o cancelamento. Para isso, é necessária a presença do gerente e, portanto, o ProxyCancelamento fica na frente da classe real e espera a senha desse gerente foi informada. Se for, o comando de cancelamento é repassado para o sistema, qeu realiza a operação.
5. Decorator: se o cliente quiser um hambúrguer com complementos extras, usa-se o decorator para adicioná-los e somar os preços de maneira dinâmica.
6. Strategy: usado para o cálculo da taxa de entrega, dependendo da preferência do cliente. Serão implementadas três diferentes estratégias: FreteNormal (mais barato, mas demora mais), FreteRapido (mais rápido e mais caro), RetirarBalcao (não cobrado), todas a partir da interface IClaculoFrete.
7. Observer: como o sujeito principal é o pedido, teremos dois observadores: o AppCliente e o PainelCozinha. Quando o status do pedido é modificado, ambos recebem notificações e atualizam as telas.
